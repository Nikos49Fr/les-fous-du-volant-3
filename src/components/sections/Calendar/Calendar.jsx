...
export default function Calendar() {
  ...
  return (
    <section className="app-section app-calendar">
      ...
      <p>
        Chaque circuit sera révélé à la fin du circuit précédent, par une
        énigme à résoudre.
      </p>
      <p>Voici les dates des Grand Prix pour la saison 3 des fous de volant :</p>
      <ol className="app-calendar__gp-list">
        {GP_DATES.map((gpDate, index) => {
          ...
          const datePart = gpDate.label.replace(` ${timePart}`, '');

          return (
            <li className="app-calendar__gp-item" key={gpDate.id}>
              <span className="app-calendar__gp-item-number">{gpDate.id}</span>
              ...
              <span
                className={`app-calendar__gp-item-name${
                  isKnown ? '' : ' app-calendar__gp-item-name--unknown'
                }`}
              >
                {isKnown
                  ? gp.name
              ...
              >
                ...
                <span className="app-calendar__gp-item-time">{timePart}</span>
              </time>
            </li>
          );
        })}
