...
function isManualRefreshCoolingDown(snapshot: { updated_at: string | null | undefined }) {
  if (!snapshot?.updated_at) {
    return false;
  }

  return Date.now() - Date.parse(snapshot.updated_at) < MANUAL_REFRESH_COOLDOWN_MS;
}

async function fetchSnapshot(adminClient: ReturnType<typeof createClient>) {
  const { data, error } = await adminClient
    .from('multi_twitch_live_snapshot')
    .select('id, live_channels, refresh_status, refresh_trigger, refresh_started_at, updated_at, last_error')
    .eq('id', SNAPSHOT_ID)
    .single();
...
async function fetchTrackedChannels(adminClient: ReturnType<typeof createClient>) {
...
async function fetchLiveChannels(logins: string[], clientId: string, accessToken: string) {
...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !supabaseServiceRoleKey || !twitchClientId || !twitchClientSecret) {
  ...
  try {
    ...
    if (snapshot.refresh_status === 'running') {
      const refreshStartedAt = snapshot.refresh_started_at ? Date.parse(snapshot.refresh_started_at) : 0;
      if (Date.now() - refreshStartedAt < RUNNING_STALE_MS) {
        return jsonResponse(200, { refreshed: false, snapshot });
      }
    }

    if (requestedTrigger === 'manual' && isManualRefreshCoolingDown(snapshot)) {
      return jsonResponse(200, { refreshed: false, snapshot });
    }
    ...
    const staleBefore = new Date(Date.now() - SNAPSHOT_TTL_MS).toISOString();
    const runningStaleBefore = new Date(Date.now() - RUNNING_STALE_MS).toISOString();
    ...
    const liveChannels = await fetchLiveChannels(trackedLogins, twitchClientId, accessToken);
    ...
    return jsonResponse(200, { refreshed: true, snapshot: updatedSnapshot });
  } catch (error) {
